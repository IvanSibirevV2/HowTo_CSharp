    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            new HttpListener()
                ...
                {
                    Console.WriteLine($"адрес приложения: {a.Request.LocalEndPoint}");
                    Console.WriteLine($"адрес клиента: {a.Request.RemoteEndPoint}");
                    Console.WriteLine(a.Request.RawUrl);
                    Console.WriteLine($"Запрошен адрес: {a.Request.Url}");
                    Console.WriteLine("Заголовки запроса:");
                    foreach (string item in a.Request.Headers.Keys)
                    {
                        Console.WriteLine($"{item}:{a.Request.Headers[item]}");
                    }

                    a.Response.Set_Bytes(
@"<!DOCTYPE html>
<html>
    <head>
        <meta charset='utf8'>
